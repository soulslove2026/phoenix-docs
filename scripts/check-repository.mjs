...
const required = [
  ...
  "FILE_MANIFEST.json", "CHECKSUMS.sha256", "releases/v3.3.3-constitutional-reconciliation-verification.md"
];
...
const expectedDocs = "3.3.3-constitutional-reconciliation-verification";
const authorityFiles = ["README.md", "RELEASE.md", "RELEASE_REPORT.md", "PROJECT_BOARD.md", "ENGINEERING_ROADMAP.md"];

if (version.version !== expectedDocs) fail("VERSION.json does not identify v3.3.3");
if (version.status !== "verified") fail("VERSION.json must record verified status");
if (version.production_ready !== false) fail("Documentation must not claim production readiness");

for (const file of authorityFiles) {
  const content = read(file);
  if (!content.includes(expectedCore) && !content.includes(expectedDocs) && !content.includes("v3.3.3")) {
    fail(`${file} does not identify the current release`);
  }
  if (!content.includes("Verified") && !content.includes("verified")) {
    fail(`${file} does not preserve Verified status`);
  }
}

if (manifest.repository_version !== version.version) fail("FILE_MANIFEST version differs from VERSION.json");
if (!read("MASTER_INDEX.md").includes("releases/v3.3.3-constitutional-reconciliation-verification.md")) fail("MASTER_INDEX current release is stale");
