...
const required=["VERSION.json","README.md","RELEASE.md","RELEASE_REPORT.md","MASTER_INDEX.md","DOCUMENT_INDEX.md","TRACEABILITY_MATRIX.md","GOVERNANCE_CHECKLIST.md","CONSTITUTION_COMPLIANCE_REPORT.md","REPOSITORY_INTEGRITY_REPORT.md","TECHNOLOGY_STACK.json","FILE_MANIFEST.json","CHECKSUMS.sha256","releases/v3.6.1-identity-slice2-phase-c-typecheck-hotfix-r1.md",...Array.from({length:10},(_,i)=>`docs/15_Identity_Slice2/${String(i+25).padStart(3,"0")}_${["Phase_C_Charter","Real_Browser_Passkey_Validation","Production_Notification_Provider_Validation","Operations_Monitoring_and_Alerting","Identity_Key_Rotation","Backup_Restore_and_Recovery","Incident_Response_and_Forensics","Supply_Chain_Provenance","Phase_C_ASVS_and_NIST_Traceability","Phase_C_Evidence_and_Exit_Gates"][i]}.md`)];
...
if(version.version!=="3.6.1-identity-slice2-phase-c-typecheck-hotfix-r1"||version.status!=="candidate"||version.production_ready!==false)fail("Release authority invalid");
if(stack.version!=="3.6.1"||!stack.production_assurance)fail("Technology stack authority stale");
for(const file of ["README.md","RELEASE.md","RELEASE_REPORT.md"]){const text=read(file);if(!text.includes("3.6.1")||!text.toLowerCase().includes("candidate"))fail(`${file} authority stale`);}
...
if(!read("MASTER_INDEX.md").includes("releases/v3.6.1-identity-slice2-phase-c-typecheck-hotfix-r1.md"))fail("MASTER_INDEX current release stale");
